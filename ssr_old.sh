...
JQ_install(){
	...
	if [[ -z ${JQ_ver} ]]; then
		...
		wget --no-check-certificate -N "https://raw.githubusercontent.com/alexafk/doubi/master/other/jq-1.5.tar.gz"
...
installServerSpeeder(){
	...
	cd /root
	#借用91yun.rog的开心版锐速
	wget -N --no-check-certificate https://raw.githubusercontent.com/alexafk/serverspeeder/master/serverspeeder-all.sh
...
BanBTPTSPAM(){
	wget -4qO- raw.githubusercontent.com/alexafk/doubi/master/Get_Out_Spam.sh | bash
}
InstallBBR(){
